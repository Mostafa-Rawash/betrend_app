(() => {
    ...
    "frappe/public/js/frappe/views/treeview.js"(exports, module) {
      ...
      frappe.views.TreeView = class TreeView {
        ...
        make_page() {
          ...
          frappe.breadcrumbs.add(me2.opts.breadcrumb || locals.DocType[me2.doctype].module, me2.doctype);
          this.set_title();
          this.page.main.css({
            "min-height": "300px"
          });
        ...
        rebuild_tree() {
          ...
          frappe.call({
            ...
            "args": {
              "doctype": me2.doctype,
              "parent_field": "parent_" + me2.doctype.toLowerCase().replace(/ /g, "_")
            },
            "callback": function(r) {
              if (!r.exc) {
                me2.make_tree();
        ...
        select_node(node) {
          ...
          if (this.opts.view_template) {
            ...
            $(frappe.render_template(me2.opts.view_template, { data: node.data, doctype: me2.doctype })).appendTo(this.node_view);
          }
        }
        get_toolbar() {
          var me2 = this;
        ...
        set_primary_action() {
          ...
          if (!this.opts.disable_add_node && this.can_create) {
            me2.page.set_primary_action(__("New"), function() {
              me2.new_node();
            }, "add");
          }
        }
  ...
  // frappe-html:/home/frappe/dev/app/apps/frappe/frappe/public/js/frappe/ui/listing.html
...
`;
  ...
  frappe.get_indicator = function(doc, doctype) {
    ...
    var is_submittable = frappe.model.is_submittable(doctype), workflow_fieldname = frappe.workflow.get_state_fieldname(doctype);
  ...
  frappe.ui.Filter = class {
    ...
    make_tag() {
      if (!this.field)
        return;
      this.$filter_tag = this.get_filter_tag_element().insertAfter(this.parent.find(".active-tag-filters .clear-filters"));
  ...
  frappe.ui.FilterGroup = class {
    ...
    update_filter_button() {
      ...
      const button_label = filters_applied ? this.filters.length > 1 ? __("{0} filters", [this.filters.length]) : __("{0} filter", [this.filters.length]) : __("Filter");
  ...
  // frappe-html:/home/frappe/dev/app/apps/frappe/frappe/public/js/frappe/ui/filters/edit_filter.html
...
`;
    ...
    $parent.on("mouseover", selector, function() {
      ...
      target_element.popover({
        ...
        content: () => {
          ...
          liked_by.forEach((user2) => {
            liked_by_list.append(`
						<li data-user=${user2}>${frappe.avatar(user2, "avatar-xs")}
							<span>${frappe.user.full_name(user2)}</span>
						</li>
					`);
          });
          liked_by_list.children("li").click((ev) => {
            let user2 = ev.currentTarget.dataset.user;
            target_element.popover("hide");
            frappe.set_route(link_base + user2);
          });
  ...
  // frappe-html:/home/frappe/dev/app/apps/frappe/frappe/public/html/print_template.html
...
`;
  ...
  frappe.views.BaseList = class BaseList {
    ...
    set_default_secondary_action() {
      if (this.secondary_action) {
        const $secondary_action = this.page.set_secondary_action(this.secondary_action.label, this.secondary_action.action, this.secondary_action.icon);
  ...
  var FilterArea = class {
    ...
    set_standard_filter(filters) {
      ...
      const fields_dict = this.list_view.page.fields_dict;
      let out = filters.reduce((out2, filter) => {
        const [dt, fieldname, condition, value] = filter;
        out2.promise = out2.promise || Promise.resolve();
        out2.non_standard_filters = out2.non_standard_filters || [];
        if (fields_dict[fieldname] && (condition === "=" || condition === "like")) {
          out2.promise = out2.promise.then(() => fields_dict[fieldname].set_value(value));
        } else {
          out2.non_standard_filters.push(filter);
        }
        return out2;
      }, {});
      return out;
    ...
    make_standard_filters() {
      ...
      const has_existing_filters = this.list_view.filters && this.list_view.filters.length > 0;
      fields = fields.concat(doctype_fields.filter((df) => df.fieldname === title_field || df.in_standard_filter && frappe.model.is_value_type(df.fieldtype)).map((df) => {
  ...
  var BulkOperations = class {
    ...
    export(doctype, docnames) {
      frappe.require("data_import_tools.bundle.js", () => {
        ...
        data_exporter.filter_group.add_filters_to_filter_group([[doctype, "name", "in", docnames, false]]);
      });
    }
  };
  ...
  var ListSettings = class {
    ...
    remove_fields(fieldname) {
      ...
      me2.set_removed_fields(me2.get_removed_listview_fields(me2.fields.map((f) => f.fieldname), existing_fields));
      me2.refresh();
      me2.update_fields();
    }
    update_fields() {
    ...
    column_selector() {
      ...
      let d = new frappe.ui.Dialog({
        ...
        fields: [
          ...
          },
          {
            ...
            fieldname: "fields",
            options: me2.get_doctype_fields(me2.meta, me2.fields.map((f) => f.fieldname)),
            columns: 2
          }
        ]
      });
      d.set_primary_action(__("Save"), () => {
        ...
        me2.set_removed_fields(me2.get_removed_listview_fields(values, me2.fields.map((f) => f.fieldname)));
        me2.fields = [];
    ...
    set_list_view_fields(meta) {
      ...
      meta.fields.forEach((field) => {
        if (field.in_list_view && !in_list(frappe.model.no_value_type, field.fieldtype) && me2.subject_field.fieldname != field.fieldname) {
          me2.fields.push({
            label: field.label,
            fieldname: field.fieldname
          });
  ...
  frappe.views.ListView = class ListView extends frappe.views.BaseList {
    static load_last_view() {
      ...
      if (route.length === 2) {
        ...
        frappe.set_route("list", frappe.router.doctype_layout || doctype, frappe.views.is_valid(last_view) ? last_view.toLowerCase() : "list");
        return true;
      }
      return false;
    }
    ...
    show_restricted_list_indicator_if_applicable() {
      ...
      if (match_rules_list.length) {
        this.restricted_list = $(`<button class="btn btn-xs restricted-button flex align-center">
					${frappe.utils.icon("restriction", "xs")}
				</button>`).click(() => this.show_restrictions(match_rules_list)).appendTo(this.page.page_form);
      }
    }
    ...
    set_fields() {
      let fields = [].concat(frappe.model.std_fields_list, this.get_fields_in_list_view(), [this.meta.title_field, this.meta.image_field], this.settings.add_fields || [], this.meta.track_seen ? "_seen" : null, this.sort_by, "enabled", "disabled", "color");
      fields.forEach((f) => this._add_field(f));
    ...
    set_primary_action() {
      if (this.can_create) {
        ...
        this.page.set_primary_action(label, () => {
    ...
    setup_columns() {
      ...
      this.columns = this.columns.concat(fields_in_list_view.filter((df) => {
    ...
    get_no_result_message() {
      ...
      let no_result_message = filters && filters.length ? __("No {0} found", [__(this.doctype)]) : __("You haven't created a {0} yet", [__(this.doctype)]);
      let new_button_label = filters && filters.length ? __("Create a new {0}", [__(this.doctype)], "Create a new document from list view") : __("Create your first {0}", [__(this.doctype)], "Create a new document from list view");
    ...
    render_skeleton() {
      const $row = this.get_list_row_html_skeleton('<div><input type="checkbox" class="render-list-checkbox"/></div>');
    ...
    render_list() {
      ...
      if (this.data.length > 0) {
        this.$result.append(this.data.map((doc, i2) => {
          doc._idx = i2;
    ...
    get_meta_html(doc) {
      ...
      const comment_count = `<span class="${!doc._comment_count ? "text-extra-muted" : ""} comment-count">
    ...
    setup_realtime_updates() {
      ...
      frappe.realtime.on("list_update", (data) => {
        ...
        frappe.call(call_args).then(({ message }) => {
          if (!message)
            return;
          const data2 = frappe.utils.dict(message.keys, message.values);
    ...
    get_checked_items(only_docnames) {
      const docnames = Array.from(this.$checks || []).map((check) => cstr(unescape($(check).data().name)));
      if (only_docnames)
        return docnames;
      ...
      const bulk_cancel = () => {
        return {
          ...
          action: () => {
            ...
            if (docnames.length > 0) {
              frappe.confirm(__("Cancel {0} documents?", [docnames.length], "Title of confirmation dialog"), () => {
                this.disable_list_update = true;
      ...
      const bulk_submit = () => {
        return {
          ...
          action: () => {
            const docnames = this.get_checked_items(true);
            if (docnames.length > 0) {
              frappe.confirm(__("Submit {0} documents?", [docnames.length], "Title of confirmation dialog"), () => {
                this.disable_list_update = true;
  ...
  frappe.views.ListViewSelect = class ListViewSelect {
    ...
    add_view_to_menu(view, action) {
      let $el = this.page.add_custom_menu_item(this.parent, __(view), action, true, null, this.icon_map[view] || "list");
    ...
    setup_views() {
      const views = {
        ...
        Kanban: {
          ...
          current_view_handler: () => {
            frappe.views.KanbanView.get_kanbans(this.doctype).then((kanbans) => this.setup_kanban_switcher(kanbans));
          }
        },
        Map: {
          condition: this.list_view.settings.get_coords_method || this.list_view.meta.fields.find((i2) => i2.fieldname === "latitude") && this.list_view.meta.fields.find((i2) => i2.fieldname === "longitude") || this.list_view.meta.fields.find((i2) => i2.fieldname === "location" && i2.fieldtype == "Geolocation"),
    ...
    setup_kanban_switcher(kanbans) {
      const kanban_switcher = this.page.add_custom_button_group(__("Select Kanban"), null, this.list_view.$filter_section);
      kanbans.map((k) => {
        this.page.add_custom_menu_item(kanban_switcher, k.name, () => this.set_route("kanban", k.name), false);
      });
      this.page.add_custom_menu_item(kanban_switcher, __("Create New Kanban Board"), () => frappe.views.KanbanView.show_kanban_dialog(this.doctype), true);
    }
    get_page_name() {
    ...
    get_email_accounts() {
      ...
      accounts.forEach((account) => {
        ...
        let display_name = [
          ...
          "Spam",
          "Trash"
        ].includes(account.email_id) ? __(account.email_id) : account.email_account;
  ...
  frappe.views.ListSidebar = class ListSidebar {
    ...
    setup_views() {
      ...
      if (this.list_view.settings.get_coords_method || this.list_view.meta.fields.find((i2) => i2.fieldname === "latitude") && this.list_view.meta.fields.find((i2) => i2.fieldname === "longitude") || this.list_view.meta.fields.find((i2) => i2.fieldname === "location" && i2.fieldtype == "Geolocation")) {
  ...
  // frappe-html:/home/frappe/dev/app/apps/frappe/frappe/public/js/frappe/list/list_sidebar.html
...
`;
  ...
  // frappe-html:/home/frappe/dev/app/apps/frappe/frappe/public/js/frappe/list/list_sidebar_stat.html
...
`;
  ...
  frappe.views.ListGroupBy = class ListGroupBy {
      ...
      this.$wrapper.find(".group-by-field").on("show.bs.dropdown", (e) => {
        ...
        this.get_group_by_count(fieldname).then((field_count_list) => {
          if (field_count_list.length) {
            let applied_filter = this.list_view.get_filter_value(fieldname == "assigned_to" ? "_assign" : fieldname);
            this.render_dropdown_items(field_count_list, fieldtype, $dropdown, applied_filter);
    ...
    set_empty_state($dropdown) {
      $dropdown.html(`<div class="empty-state group-by-empty">
				${__("No filters found")}
			</div>`);
    }
  ...
  // frappe-html:/home/frappe/dev/app/apps/frappe/frappe/public/js/frappe/list/list_view_permission_restrictions.html
...
`;
      ...
      this.cal_options = {
        ...
        select: function(startDate, endDate, jsEvent, view) {
          ...
          if (me2.field_map.allDay) {
            ...
            if (all_day)
              event[me2.field_map.end] = me2.get_system_datetime(moment(endDate).subtract(1, "s"));
          }
    ...
    get_update_args(event) {
      ...
      if (this.field_map.end) {
        ...
        if (args[this.field_map.allDay]) {
          args[this.field_map.end] = me2.get_system_datetime(moment(event.end).subtract(1, "s"));
        }
      }
  ...
  frappe.views.DashboardView = class DashboardView extends frappe.views.ListView {
    ...
    add_customization_buttons() {
      this.save_customizations_button = this.page.add_button(__("Save Customizations"), () => {
    ...
    make_dashboard() {
      if (this.dashboard_settings) {
        ...
        frappe.run_serially([
          () => this.fetch_dashboard_items("Dashboard Chart", {
  ...
  frappe.views.ImageView = class ImageView extends frappe.views.ListView {
    ...
    item_details_html(item) {
      ...
      info_fields.forEach((field, index) => {
        if (item[field] && !set) {
          if (index == 0)
            info_html += `<li>${item[field]}</li>`;
          else
            info_html += `<li class="text-muted">${item[field]}</li>`;
          set = true;
        }
      });
      info_html += `</ul></div>`;
  ...
  frappe.views.MapView = class MapView extends frappe.views.ListView {
    ...
    render_map_view() {
      ...
      if (this.coords.features && this.coords.features.length) {
        this.coords.features.forEach((coords) => L.geoJSON(coords).bindPopup(coords.properties.name).addTo(this.map));
    ...
    get_coords() {
      ...
      if (cur_list.meta.fields.find((i2) => i2.fieldname === "location" && i2.fieldtype === "Geolocation")) {
  ...
  var KanbanSettings = class {
    ...
    get_multiselect_fields() {
      ...
      return frappe.model.std_fields.concat(this.kanbanview.get_fields_in_list_view()).filter((field) => !ignore_fields.includes(field.fieldname) && !ignore_fieldtypes.includes(field.fieldtype)).map((field) => {
        return {
  ...
  frappe.views.KanbanView = class KanbanView extends frappe.views.ListView {
    ...
    get_card_meta() {
      ...
      this.meta.fields.forEach((df) => {
        const is_valid_field = in_list(["Data", "Text", "Small Text", "Text Editor"], df.fieldtype) && !df.hidden;
  ...
  frappe.views.KanbanView.show_kanban_dialog = function(doctype, show_existing) {
    ...
    function new_kanban_dialog(kanbans, show_existing2) {
      if (dialog)
        return dialog;
      const dialog_fields = get_fields_for_dialog(kanbans.map((kanban) => kanban.name), show_existing2);
      ...
      let primary_action = () => {
        if (to_save) {
          ...
          if (!values.selected_kanban || values.selected_kanban == "Create New Board") {
            make_kanban_board(values.board_name, values.field_name, values.project).then(() => dialog.hide(), (err) => frappe.msgprint(err));
          } else {
    ...
    function get_fields_for_dialog(kanban_options, show_existing2 = false) {
      ...
      let fields = [
        {
          ...
          reqd: 1,
          depends_on: `eval: ${show_existing2}`,
          mandatory_depends_on: `eval: ${show_existing2}`,
        ...
        },
        {
          ...
          fieldtype: "Section Break",
          depends_on: `eval: !${show_existing2} || doc.selected_kanban == "Create New Board"`
        },
        {
          fieldtype: "Data",
  ...
  frappe.views.InboxView = class InboxView extends frappe.views.ListView {
    ...
    get_seen_class(doc) {
      const seen = Boolean(doc.seen) || JSON.parse(doc._seen || "[]").includes(frappe.session.user) ? "" : "bold";
      return seen;
    }
    get is_sent_emails() {
  ...
  frappe.views.FileView = class FileView extends frappe.views.ListView {
          ...
          action: () => {
            new frappe.ui.FileUploader({
              ...
              on_success: (file) => {
                ...
                frappe.call("frappe.core.doctype.file.file.unzip_file", {
                  name: file.name
                }).then((r) => {
                  if (r.message) {
                    frappe.show_alert(__("Unzipped {0} files", [
                      r.message
                    ]));
                  }
                });
    ...
    render_grid_view() {
      ...
      this.$result.empty().html(`<div class="file-grid">
				${html}
			</div>`);
    }
    ...
    get_header_html() {
      ...
      let header_selector_html = !frappe.views.FileView.grid_view ? `<input class="level-item list-check-all hidden-xs" type="checkbox" title="${__("Select All")}">` : "";
    ...
    setup_drag_events() {
      this.$result.on("dragstart", ".files .file-wrapper", (e) => {
        ...
        frappe.file_manager.cut([{ name: $(e.currentTarget).attr("data-name") }], this.current_folder);
      });
      this.$result.on("dragover", (e) => {
        e.preventDefault();
      }, false);
  ...
  frappe.views.InteractionComposer = class InteractionComposer {
    ...
    setup_attach() {
      ...
      $("<h6 class='text-muted add-attachment' style='margin-top: 12px; cursor:pointer;'>" + __("Select Attachments") + "</h6><div class='attach-list'></div>			<p class='add-more-attachments'>			<a class='text-muted small'><i class='octicon octicon-plus' style='font-size: 12px'></i> " + __("Add Attachment") + "</a></p>").appendTo(attach.empty());
    ...
    create_action() {
      ...
      if (!form_values)
        return;
      let selected_attachments = $.map($(me2.dialog.wrapper).find("[data-file-name]:checked"), function(element) {
  ...
  // frappe-html:/home/frappe/dev/app/apps/frappe/frappe/public/js/frappe/views/image/image_view_item_row.html
...
`;
  ...
  // frappe-html:/home/frappe/dev/app/apps/frappe/frappe/public/js/frappe/views/image/photoswipe_dom.html
...
<div class="pswp" tabindex="-1" role="dialog" aria-hidden="true">
  ...
  // frappe-html:/home/frappe/dev/app/apps/frappe/frappe/public/js/frappe/views/kanban/kanban_board.html
  ...
  // frappe-html:/home/frappe/dev/app/apps/frappe/frappe/public/js/frappe/views/kanban/kanban_column.html
  ...
  // frappe-html:/home/frappe/dev/app/apps/frappe/frappe/public/js/frappe/views/kanban/kanban_card.html
...
//# sourceMappingURL=list.bundle.33X7DLAY.js.map
